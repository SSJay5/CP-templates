...
ll fun(string a, string b, v<v<ll>> &dp)
{
	...
	if (a.back() == b.back())
	{
		ans1 = 1 + fun(a.substr(0, a.length() - 1), b.substr(0, b.length() - 1), dp);
	}
	ans2 = fun(a.substr(0, a.length() - 1), b, dp);
	ans3 = fun(a, b.substr(0, b.length() - 1), dp);
	dp[a.length() - 1][b.length() - 1] = max(ans1, max(ans2, ans3));
